...
class TodoListApp(MDApp):
    ...
    def save_tasks(self):
        for i, task_data in enumerate(self.tasks):
            self.store.put(str(i), text=task_data['text'], completed=task_data['completed'])

    def load_tasks(self):
        self.tasks = []
        for key in self.store.keys():
            task_data = self.store.get(key)
            self.tasks.append({"text": task_data['text'], "completed": task_data['completed']})
